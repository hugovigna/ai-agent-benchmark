...
\begin{tabular}{cll}
  ...
  \# & Titre & Branche \\
  \midrule
  2 & Replace Hardcoded Credentials     & \code{challenge/hardcoded-creds} \\
  3 & Refactoring Pipeline Monolithique & \code{challenge/monolith-pipeline} \\
  4 & Checkpointing Pipeline            & \code{challenge/add-checkpointing} \\
  5 & Module Qualité de Données         & \code{challenge/data-quality} \\
  6 & Rétrodocumentation                & \code{challenge/retrodoc} \\
  7 & Mapping Codebase                  & \code{challenge/mapping} \\
  8 & Debugging (4 bugs)                & \code{challenge/debugging} \\
  9 & Pipeline Pandas (bugs silencieux) & \code{challenge/pandas-pipeline} \\
  \bottomrule
\end{tabular}
\end{center}

\textit{Note : le challenge 1 (merge conflict) est manuel et non automatisé.}
...
validate_challenge8.py    Validator challenge 8 (debugging)
validate_challenge9.py    Validator challenge 9 (pandas pipeline)
...
Utilisé dans : challenge 8 (debugging), challenge 9 (pandas pipeline).
...
% ------------------------------------------------------------------------------
\subsection{Challenge 2 — Replace Hardcoded Credentials}
...
% ------------------------------------------------------------------------------
\subsection{Challenge 3 — Refactoring Pipeline Monolithique}
...
% ------------------------------------------------------------------------------
\subsection{Challenge 4 — Checkpointing Pipeline}
...
% ------------------------------------------------------------------------------
\subsection{Challenge 5 — Module de Qualité de Données}
...
% ------------------------------------------------------------------------------
\subsection{Challenge 6 — Rétrodocumentation}
...
% ------------------------------------------------------------------------------
\subsection{Challenge 7 — Mapping Codebase}
...
% ------------------------------------------------------------------------------
\subsection{Challenge 8 — Debugging (4 bugs)}
\textit{Branche :} \code{challenge/debugging} \quad \textit{Difficulté : ★★★}

\textbf{Situation initiale :} 4 fichiers Python, chacun avec un type de bug différent.

\begin{center}
\begin{tabular}{lll}
  \toprule
  Fichier & Type de bug & Critère de succès \\
  \midrule
  \code{bug\_type\_error.py}    & Types incompatibles (list/array/Series) & Retourne le dict attendu \\
  \code{bug\_infinite\_loop.py} & Boucle ne convergeant pas                & Termine en $< 5$ secondes \\
  \code{bug\_wrong\_path.py}    & Chemin de fichier incorrect               & \code{status: "ok"} \\
  \code{bug\_slow\_code.py}     & Code non vectorisé (boucle sur 100k lignes) & Correct en $< 2$ secondes \\
  \bottomrule
\end{tabular}
\end{center}

\textbf{Checks du validator ($\sim$12 checks) :}
Pour chaque fichier : import + exécution du point d'entrée, vérification du
type et des valeurs retournées, vérification du timing pour les fichiers 2 et 4.

\textit{Note : le validator exécute réellement le code.
Dépendances requises : \code{pip install pandas numpy}}

% ------------------------------------------------------------------------------
\subsection{Challenge 9 — Pipeline Pandas (bugs silencieux)}
...
\begin{tabular}{clr}
  ...
  \midrule
  2 & Hardcoded Credentials     & 8 (fixes) \\
  3 & Monolith Pipeline         & $\sim$10--12 (variable) \\
  4 & Add Checkpointing         & 8 (fixes) \\
  5 & Data Quality              & $\sim$12--14 (fixes) \\
  6 & Rétrodocumentation        & variable (dynamique) \\
  7 & Mapping Codebase          & $\sim$36 (fixes) \\
  8 & Debugging                 & $\sim$12 (fixes) \\
  9 & Pandas Pipeline           & 7 (fixes) \\
  \bottomrule
\end{tabular}
\end{center}
...
\textbf{Score partiel sur challenge 6} $\to$ normal, le nombre de checks est
